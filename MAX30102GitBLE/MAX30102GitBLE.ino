 /********************************************************
  ...
  Projecsadfsadfsadfsadfsadfsadfsadfsadfsadfsadfdsft: MAXREFDES117#
...
//global varbiles which will store their corresponding values that will be used for values to write over ble
String spo2, heartRate, surfaceTemp;

//BLE Service Name
BLEService customService("13012F00-F8C3-4F4A-A8F4-15CD926DA146");
BLEStringCharacteristic ble_heartRate("13012F01-F8C3-4F4A-A8F4-15CD926DA146", BLERead | BLENotify, 20);
BLEStringCharacteristic ble_spo2("13012F02-F8C3-4F4A-A8F4-15CD926DA146", BLERead| BLENotify, 20);
BLEStringCharacteristic ble_temp("13012F03-F8C3-4F4A-A8F4-15CD926DA146", BLERead | BLENotify, 20);
...
void getData(); //forward decleration
...
void setup() {
  ...
  if (!BLE.begin()) {
    ...
    while (1){
        ...
        //probably do not want a loop here likely want to force a hardware reset
    }
  }
  digitalWrite(LED_PWR, LOW);
  BLE.setLocalName("Nano33 with MAXREFDES");
  //create Custom Service
  BLE.setAdvertisedService(customService);
  //create characteristics to add to the custom service
  customService.addCharacteristic(ble_heartRate);
  customService.addCharacteristic(ble_spo2);
  customService.addCharacteristic(ble_temp);

  BLE.addService(customService);
  //start sending signals for central to find this device
  BLE.advertise();
...
#endif // USE_ADALOGGER
                   old_n_spo2=n_spo2;
                   heartRate = (String) n_heart_rate + " B/M";
                   Serial.println();
                   Serial.println("Heart Rate loaded into BLE");
                   Serial.print(heartRate);
                   Serial.println();
                   spo2 = (String) n_spo2 + " %";
                   Serial.println();
                   Serial.println("SpO2 loaded into BLE");
                   Serial.print(spo2);
                   Serial.println();
                   surfaceTemp = (String) temperature + " C";
                   Serial.println();
                   Serial.println("Temp loaded into BLE");
                   Serial.print(temperature);
                   Serial.println();
                   digitalWrite(LED_PWR, HIGH);
                   ble_spo2.writeValue(spo2);
                   ble_heartRate.writeValue(heartRate);
                   ble_temp.writeValue(surfaceTemp);
                   digitalWrite(LED_PWR, LOW);
                 }
                 }
